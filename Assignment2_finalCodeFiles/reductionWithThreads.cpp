...
int main()
{
  ...
  double start_time_block1 = omp_get_wtime();
...
// Print employees and their working hours safely
  ...
  #pragma omp parallel for num_threads(2) reduction(+:total[:days]) ordered
  for (int j = 0; j < days; j++) {
    #pragma omp ordered
  {
...
// Calculate salary
#pragma omp parallel for num_threads(2) reduction(+:total_hours)
    for (int i = 0; i < days; i++) {
         int Current_Thread = omp_get_thread_num();  // Get the current thread number
         int Total_Threads = omp_get_num_threads(); // Get the total number of thread
